...
class AdminController extends Controller
{
    public function allCourses()
    {
        ...
        $pastCourses = Course::where('ends', '<', Carbon::now()->format('Y-m-d H:m:s'))->get();
        return view('admin.courses', ['courses' => $courses,'pastCourses' => $pastCourses]);
    }

    public function showAllEvents()
    {
        $events = Event::where('to', '>', Carbon::now()->format('Y-m-d H:m:s'))->get();
        $pastEvents = Event::where('to', '<', Carbon::now()->format('Y-m-d H:m:s'))->get();
