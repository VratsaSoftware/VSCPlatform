            ...
            <div class="col-md-12 d-flex flex-row flex-wrap">
                <div class="col-md-12 level-title-holder d-flex flex-row flex-wrap">
                    <div class="col-md-12 text-center" style="margin-top:7vw;">
                        @if(!env('HIDE_APPLICATION_FORM'))
                        <p>
                            <p>Този формуляр е първата стъпка за кандидатстване, с която не само да се запишете, но и да покажете готовността си да се обучавате.</p>
                        <p>
                            <hr>
                        ...
                        <form action="{{route('application.store')}}" method="POST" class="col-md-12" id="application" name="application" enctype="multipart/form-data">
                                    ...
                                    <select class="section-el-bold col-md-8" name="course" id="course-select">
                                        @if(!is_null($applicationFor))
                                            @foreach($applicationFor as $position => $course)
                                                <option value="{{$course->id}}" {{$position < 1?'selected':''}}>{{strpos($course->name, 'предпочитания')?substr($course->name, 0, -9):$course->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                @endif
                            ...
                            <!--</p>-->
                            <p>
                                <label for="accompliments">Разкажете ни за 3 постижения, с които се гордеете и защо. (те могат да бъдат в личен, професионален план или свързани с учене) <span id="accompliments-label"></span><span class="req-star-form">*</span></label>
                        ...
                        @else
                        <p> <p>Кандидатстването за сезон 2020/2021г на курсовете приключи!<br/> Очаквайте да отворим кандидатстване за следващ сезон курсове.</p> <a href="{{route('home')}}"><button class="btn btn-success">към начало</button></a></p>
                        @endif
                    </div>
                </div>
            </div>
